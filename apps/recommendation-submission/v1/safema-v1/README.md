# Recommendation Submission v1 — SafeMA v1 integration
...
before the raw effect. The frozen Base App in `../base-app/` is unchanged. The
reusable runtime itself lives at the repository root in `safema-v1/`.
...
Three trusted metadata inputs or events exist in this local prototype:
...
2. Request-source adapters listed in `trusted-origins-v1.yaml`. Deployment is
   ...
   ADD/CANCEL/REPLACE events mint or transition trusted Context metadata.
3. A modeled successful external return may drive a generic, model-declared
   Context attribute update. In RecSub, success transitions the authorizing
   Context from `ACTIVE` to `SUBMITTED`.
...
`applicant`, `purpose`, `state`, `channel`, and `allowed_destinations` are
...
python apps/recommendation-submission/v1/safema-v1/runner.py \
  --metadata-db ./safema.sqlite3 -- \
...
```bash
PYTHONPATH=safema-v1:apps/recommendation-submission/v1/base-app \
  python -m pytest -q apps/recommendation-submission/v1/safema-v1/tests
PYTHONPATH=safema-v1:apps/recommendation-submission/v1/base-app \
  python apps/recommendation-submission/v1/safema-v1/evaluation/run_v1.py
```
