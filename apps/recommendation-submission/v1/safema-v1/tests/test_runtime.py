...
EFFECTS = INTEGRATION_ROOT / "models/api-effects-v1.yaml"
ORIGINS = INTEGRATION_ROOT / "models/trusted-origins-v1.yaml"
POLICY = INTEGRATION_ROOT / "policies/recommendation-disclosure-v1.yaml"
...
def force_base_app_pending(app: Application, *, forget_submissions: bool = False) -> None:
    """Simulate stale/buggy Base App state without changing trusted SafeMA state."""
    with app.repository.transaction() as connection:
        connection.execute(
            "UPDATE requests SET status = 'PENDING' WHERE request_id = 'REQ-000001'"
        )
        if forget_submissions:
            connection.execute(
                "DELETE FROM submissions WHERE request_id = 'REQ-000001'"
            )
...
def test_trusted_cancel_transitions_to_canceled_and_denies_stale_base_app(
    tmp_path, channel, destination, raw_attribute
):
    guard = runtime(tmp_path)
    app, email, portal = application(
    ...
    try:
        prepare(app, tmp_path)
        source = app.components.request_sources[0]
        ...
        assert app.sync().applied
        assert guard.registry.all_contexts()[0].attributes["state"] == "CANCELED"

        force_base_app_pending(app)
        report = app.process_pending()

        adapter = email if channel == "EMAIL" else portal
        assert len(getattr(adapter, raw_attribute)) == 0
        assert report.failed and not report.succeeded
        decision = guard.registry.decisions()[-1]
        assert decision["decision"] == "DENY"
        assert decision["raw_invoked"] == 0
...
def test_success_transitions_to_submitted_and_denies_duplicate_base_app_attempt(
    tmp_path, channel, destination, raw_attribute
):
    guard = runtime(tmp_path)
    app, email, portal = application(
    ...
    try:
        ...
        assert first.succeeded and len(getattr(adapter, raw_attribute)) == 1
        assert guard.registry.all_contexts()[0].attributes["state"] == "SUBMITTED"

        force_base_app_pending(app, forget_submissions=True)
        second = app.process_pending()

        assert second.failed and not second.succeeded
        assert len(getattr(adapter, raw_attribute)) == 1
        decision = guard.registry.decisions()[-1]
        assert decision["decision"] == "DENY"
        assert decision["raw_invoked"] == 0
...
def test_failed_external_result_leaves_lifecycle_active_for_retry(
    tmp_path, channel, destination, raw_attribute
):
    guard = runtime(tmp_path)
    app, email, portal = application(
    ...
    try:
        prepare(app, tmp_path)
        adapter = email if channel == "EMAIL" else portal
        adapter.fail_all = True

        failed = app.process_pending()
        assert failed.failed and len(getattr(adapter, raw_attribute)) == 1
        assert guard.registry.all_contexts()[0].attributes["state"] == "ACTIVE"

        adapter.fail_all = False
        retried = app.process_pending()
        assert retried.succeeded and len(getattr(adapter, raw_attribute)) == 2
        assert guard.registry.all_contexts()[0].attributes["state"] == "SUBMITTED"
    finally:
        app.close()
        guard.close()
...
@pytest.mark.parametrize(
    "channel,destination",
    [
        ("EMAIL", "alice-submit@example.edu"),
        ("PORTAL", "https://portal.example.edu/alice"),
    ],
)
def test_raised_external_error_leaves_lifecycle_active(
    tmp_path, channel, destination
):
    guard = runtime(tmp_path)
    app, email, portal = application(
        tmp_path, event(channel=channel, destination=destination)
    ...
    try:
        prepare(app, tmp_path)
        adapter = email if channel == "EMAIL" else portal
        adapter.raise_correlation_ids.add("REQ-000001")
        ...
        assert decision["raw_outcome"] == "RAISED:RuntimeError"
    finally:
        app.close()
        guard.close()
...
def test_policy_subset_to_eq_mutation_changes_behavior(tmp_path):
    ...
    try:
        ...
        guard.registry.patch_context(
            ["trusted", "multi-destination"],
            {"state": "ACTIVE"},
...
def test_trusted_cancel_requires_active_and_sets_canceled_state(tmp_path):
    guard = runtime(tmp_path)
    ...
    try:
        ...
        assert context.attributes["state"] == "CANCELED"
    finally:
        guard.close()
...
def test_external_control_plane_and_runner_persist_across_processes(tmp_path):
    ...
    environment["PYTHONPATH"] = os.pathsep.join(
        [str(REPOSITORY_ROOT / "safema-v1"), str(APP_VERSION_ROOT / "base-app")]
    )
    runner = [
        sys.executable, str(INTEGRATION_ROOT / "runner.py"),
        "--metadata-db", str(sidecar),
