...
| modeled raw API success result | TRUSTED_EVENT | may drive a declared post-return Context update |
...
An API effect model may declare a generic Context update after the raw method
returns. SafeMA selects the authorizing trusted Context before the call and
applies the update only when the modeled return condition is true. For RecSub,
`outcome=SUCCEEDED` changes `state=ACTIVE` to `state=SUBMITTED`; a failed result
or raised exception leaves the state unchanged.
