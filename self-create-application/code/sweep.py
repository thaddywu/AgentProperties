...
os.makedirs("../results/sweep-01-rerun/runs", exist_ok=True)

MODELS = ["gpt-5.6", "gpt-5.5"]
TASKS = ["T1", "T2", "T7"]
N = 20
lock = threading.Lock()
out = open("../results/sweep-01-rerun/results.jsonl", "a")
...
def one(job):
    ...
    try:
        p = R.episode_llm(task, seed=seed, model=model, backend="openai",
                          outdir=f"../results/sweep-01-rerun/runs/{model.replace('.','')}")
        v = checker.check(p)
