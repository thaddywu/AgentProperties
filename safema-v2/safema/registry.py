...
class MetadataRegistry:
    ...
    def patch_context(
        ...
        *,
        ...
        for name, expected in (require or {}).items():
            actual = merged.get(name)
            if actual != expected:
                raise OriginError(
                    f"context identity {identity!r} requires attribute {name!r} "
                    f"to be {expected!r}, got {actual!r}"
                )
