...
def load_api_models(path: str | Path) -> dict[str, dict[str, Any]]:
    ...
    for identifier, model in models.items():
        ...
        for index, transition in enumerate(after_return):
            where = f"model {identifier}.effect.after_return[{index}]"
            _keys(
                transition,
                {"id", "applies_when", "when", "update_context"},
                set(),
                where,
            )
            _identifier(transition["id"], f"{where}.id")
            validate_policy_expression(transition["applies_when"], f"{where}.applies_when")
            validate_policy_expression(transition["when"], f"{where}.when")
            update = transition["update_context"]
            _keys(
                update,
                {"in", "as", "where", "cardinality", "require", "set"},
                set(),
                f"{where}.update_context",
            )
            validate_policy_expression(update["in"], f"{where}.update_context.in")
            _identifier(update["as"], f"{where}.update_context.as")
            validate_policy_expression(update["where"], f"{where}.update_context.where")
            if update["cardinality"] != "one":
                raise ModelError(f"{where}.update_context.cardinality must be 'one'")
            for field in ("require", "set"):
                values = update[field]
                if not isinstance(values, dict) or not values:
                    raise ModelError(f"{where}.update_context.{field} must be a mapping")
                for name, expression in values.items():
                    _identifier(name, f"{where}.update_context.{field} key")
                    validate_value(expression, f"{where}.update_context.{field}.{name}")
    return models
...
def _validate_operation(operation: Any, where: str) -> None:
    ...
    if kind == "put_context":
        ...
        _keys(declaration, {"identity", "set"}, {"require"}, where)
        validate_value(declaration["identity"], f"{where}.identity")
        require = declaration.get("require", {})
        if not isinstance(require, dict):
            raise ModelError(f"{where}.require must be a mapping")
        for name, expression in require.items():
            _identifier(name, f"{where}.require key")
            validate_value(expression, f"{where}.require.{name}")
        ...
        for name, expression in declaration["set"].items():
            _identifier(name, f"{where}.set key")
